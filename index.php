...
$grab = ngegrab('https://www.googleapis.com/youtube/v3/search?part=snippet&order=relevance&regionCode=id&key=' . $devkey . '&playlistId=' . $id . '&maxResults=20&pageToken=' . $yesPage . '&type=music');
$json = json_decode($grab);
if ($json) {
    foreach ($json->items as $hasil) {
        $channel = $hasil->snippet->channelTitle;
    }
    $title = 'GRAMSTER.INFO - Gudangnya MP3KW 2018 100% Gratis Download Musik';
    $discription = 'GRAMSTER.INFO - Gudangnya MP3KW 2018 100% Gratis Download Musik';
    include 'tm-inc/head.php';
    echo '<h2 class="bmenu1" align="center">Kumpulan Lagu Terbaru 2018</h2>';
}
$json = json_decode($grab);
